!==========================================================================
! Program: XDAT                                              
! Version: xdat.x_1.0 (Modified to include system RMSD)
...
! Modification: Added system-wide RMSD calculation
!==========================================================================
PROGRAM  XDAT
!==========================================================================
!-----------------------------------------------------------------------
...
REAL(ap) :: system_rmsd_sum, system_rmsd  ! Added for system RMSD
REAL(ap) :: dx, dy, dz, atom_disp_sq      ! Added for displacement calculations
...
! Modified header to include system RMSD
WRITE(36,"(A)") "# Step  Element_RMSDs...  System_RMSD"
...
DO WHILE (IOstatus .eq. 0)
  ...
  ! Initialize system RMSD sum
  system_rmsd_sum = 0.0_ap
  ...
  DO k = 1, ntype
    ...
    DO m = 1, atnr(k)
      ...
      ELSE            ! calc deviations
        ...
        ! Calculate squared displacement for this atom
        atom_disp_sq = (dx*avect(1)+dy*bvect(1)+dz*cvect(1))**2 + &
                       (dx*avect(2)+dy*bvect(2)+dz*cvect(2))**2 + &
                       (dx*avect(3)+dy*bvect(3)+dz*cvect(3))**2
        
        ! Add to element-specific RMSD
        disp(k) = disp(k) + atom_disp_sq
        
        ! Add to system RMSD sum
        system_rmsd_sum = system_rmsd_sum + atom_disp_sq
      END IF
  ...
  ! Calculate system RMSD and write output
  IF(i > 1) THEN
    system_rmsd = sqrt(system_rmsd_sum / maxatom)
    WRITE(36,*) i, disp, system_rmsd
  ELSE
    WRITE(36,*) i, disp, 0.0_ap
  END IF
